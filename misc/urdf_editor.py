...
class UrdfEditor(object):
    ...
    def writeVisualShape(self, file, urdfVisual, precision=5):
        ...
        if urdfVisual.geom_type == p.GEOM_MESH:
            ...
            str = '\t\t\t\t<mesh filename=\"{}\" scale=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
                urdfVisual.geom_meshfilename, urdfVisual.geom_meshscale[
                    0], urdfVisual.geom_meshscale[1], urdfVisual.geom_meshscale[2],
                prec=precision)
            file.write(str)
    ...
    def writeJoint(self, file, urdfJoint, precision=5):
        ...
        file.write(str)
        str = '\t\t<limit effort="1000.0" lower="{:.{prec}f}" upper="{:.{prec}f}" velocity="10"/>\n'.format(
            urdfJoint.joint_lower_limit, urdfJoint.joint_upper_limit, prec=precision)
        file.write(str)
        ...
        str = '\t\t<origin rpy=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\" xyz=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(urdfJoint.joint_origin_rpy[0],
                                                                                                                               urdfJoint.joint_origin_rpy[
                                                                                                                                   1], urdfJoint.joint_origin_rpy[2],
                                                                                                                               urdfJoint.joint_origin_xyz[
                                                                                                                                   0],
                                                                                                                               urdfJoint.joint_origin_xyz[1], urdfJoint.joint_origin_xyz[2], prec=precision)
