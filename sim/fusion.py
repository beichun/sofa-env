...
from sim.utils import OBJ

class TSDFVolume:
    """Volumetric TSDF Fusion of RGB-D Images.
    """

    def __init__(self, vol_bnds, voxel_size, trunc_margin_scale=5, use_gpu=True):
        """Constructor.
        ...
        Args:
          ...
          trunc_margin_scale (float): trunc_margin = trunc_margin_scale * voxel_size
        """
        ...
        self._trunc_margin = trunc_margin_scale * self._voxel_size  # truncation on SDF
    ...
    # def get_point_cloud_sample(self, n_points, skip_z=None, upward_only=False, get_mesh=False):
    #     from misc.obj_utils import OBJ
    #     from misc.data_utils import plot_pc, plot_show, plot_pc_n
    #     verts, faces, norms, colors = self.get_mesh(skip_z=skip_z)

    #     # from misc.geometry_utils import mesh_sample_barycentric
    #     # pc, face_idx = mesh_sample_barycentric(verts, faces, n_points)

    #     obj = OBJ(np.copy(verts), np.copy(faces), vnormals=np.array([]))
    #     pc, pc_n, face_idx = obj.sample_points(n_points, with_normal=True, upward_only=upward_only)

    #     # if upward_only:
    #     #     pc1, pc_n1, face_idx1 = obj.sample_points(n_points, with_normal=True, upward_only=True)

    #     #     plot_pc(pc, color=(1, 0, 0))

    #     #     idx = np.where(pc_n[:, -1] >= 0)[0]
    #     #     plot_pc_n(np.append(pc, pc_n, axis=-1)[idx], color=(1, 0, 0))
    #     #     plot_pc(pc1, color=(0, 1, 0))
    #     #     plot_show()

    #     pc_w_n = np.append(pc, pc_n, axis=1)        

    #     if not get_mesh:
    #         return pc_w_n 
    #     else:
    #         return pc_w_n, [verts, faces, norms, colors]

    def get_mesh(self, skip_z=None):
        """Compute a mesh from the voxel volume using marching cubes.
        """
        tsdf_vol, color_vol = self.get_volume()

        mask = np.ones_like(tsdf_vol, dtype=np.bool_)
        if not(skip_z is None):
            mask[:, :, :skip_z] = 0
...
def meshwrite_obj(mesh_path, vertices, faces, normalize_pos=False):
  faces += 1
...
def tsdf2mesh(tsdf_vol, mesh_path, level=0, skip_z=None):
    ...
    if not(skip_z is None):
        mask[:, :, :skip_z] = 0
    # Marching cubes
    verts,faces,norms,vals = measure.marching_cubes(tsdf_vol,level=level, mask=mask)
    ...
    if mesh_path.endswith('.ply'):
        ...
        # outputs obj
        meshwrite_obj(mesh_path, verts, faces, True)